...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.Процедура           = "ОбновлениеИнформационнойБазыИТ.ВКМ_НастроитьОтправкуУведомленийТелеграм";
...
// Обработчик обновления: настраивает автоматическую отправку уведомлений
// телеграм-боту.
//
// Расписание предопределенного регламентного задания хранится в данных
// информационной базы, а не в конфигурации, поэтому задать его в метаданных
// нельзя: без этого обработчика задание есть, но никогда не запускается.
//
Процедура ВКМ_НастроитьОтправкуУведомленийТелеграм() Экспорт
	...
	// Очередь разбирается раз в минуту: заявка уходит в телеграм почти сразу
	// после записи документа, но частота остается в пределах лимитов сервиса.
